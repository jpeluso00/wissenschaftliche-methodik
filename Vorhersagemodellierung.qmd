...
Für die Explorative Datenanalyse und die darauf Folgenden Schritte, werden sowohl Anwendungs als auch Trainings Daten aus den zur Verfügung gestellten csv Dateien gelesen.
...
Fassen Sie kurz die zentralen Ergebnisse zusammen (0,5--1 Seite). Gehen Sie auch auf die Grenzen Ihrer Analyse ein. In der vorliegenden Ausarbeitung konnte ein Modell zur Vorhersage der Vermietungen in Abhängigkeit verschiedener Faktoren erarbeitet werden. Als Modell wurde ein neuronales Netz gewählt, welches einen MAE von (`r results$MAE[1]`) erreicht. Die Anzahl des besten Modells liegt bei 
`r results$name[1]` und hat `r results$Neurons[1]` Input Neuronen. Als Dropout Rate wurde das Modell mit `r results$dropr[1]` trainiert. Für das Training und den Vergleich der Modelle wurden verschiedene Wert der Hyperparameter genutzt. In einer weiteren Betrachtung des Themas könnte zur Optimierung der Hyperparameter per Gridsearch stattfinden. Außerdem könnte zur Erkennung von längerfristigen Wirkung von zeitlichen Ereignissen auch eine Analyse der Zeitreihen Daten stattfinden, dieses könnte sich auf das Wetter des Vortages beziehen allerdings, beispielsweise: wenn das Wetter am Vortag schlecht war und die Vermietungen niedrig werden am nächsten Tag mehr/weniger Fahrräder vermietet.
